...
export const showEstadoUsuarioId = async (req, res) => {
  try {
    const [result] = await connect.query('SELECT * FROM estado_usuario WHERE Estado_usuario_id= ?', [req.params.id]);
...
export const updateEstadoUsuario = async (req, res) => {
  try {
    ...
    let sqlQuery = "UPDATE estado_usuario SET Estado_usuario_name=?,Estado_usuario_descripcion=?,Updated_at=? WHERE Estado_usuario_id= ?";
...
export const deleteEstadoUsuario = async (req, res) => {
  try {
    let sqlQuery = "DELETE FROM estado_usuario WHERE Estado_usuario_id= ?";
